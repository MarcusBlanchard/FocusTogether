#!/usr/bin/env bash
# Build and copy Zirain.app to /Applications.
...
APP="$ROOT/src-tauri/target/release/bundle/macos/Zirain.app"
if [[ ! -d "$APP" ]]; then
  echo "error: expected bundle not found: $APP" >&2
  exit 1
fi

rm -rf /Applications/Zirain.app
cp -R "$APP" /Applications/
echo "Installed to /Applications/Zirain.app"
