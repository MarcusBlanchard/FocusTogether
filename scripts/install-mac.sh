#!/usr/bin/env bash
# Build and copy Flowlocked.app to /Applications.
...
APP="$ROOT/src-tauri/target/release/bundle/macos/Flowlocked.app"
if [[ ! -d "$APP" ]]; then
  echo "error: expected bundle not found: $APP" >&2
  exit 1
fi

rm -rf /Applications/Flowlocked.app
cp -R "$APP" /Applications/
echo "Installed to /Applications/Flowlocked.app"
